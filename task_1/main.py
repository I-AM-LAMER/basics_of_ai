...
tab1_content = [dbc.Row([    # строка графиков
        ...
        dbc.Col(html.Div(id='mstar-tstar-chart'), md=6)
                ...
                dbc.Row([
        dbc.Col(html.Div(id='tstar-rstar-chart'), md=6),
        dbc.Col(html.Div(id='mstar-rstar-chart'), md=6)
    ])]
...
@app.callback(         #для данимического изменения нашего графика
    [Output(component_id='dist-temp-chart', component_property='children'),
           ...
           Output(component_id='tstar-rstar-chart', component_property='children'),
           Output(component_id='mstar-rstar-chart', component_property='children'),
...
def update_dist_temp_chart(data):
    ...
    if len(chart_data) == 0:
        return html.Div('Please select more data'), html.Div(), html.Div(), html.Div(), html.Div()
    ...
    fig5 = px.scatter(chart_data, x='TSTAR', y='RSTAR', size='RPLANET',
                       color='status', color_discrete_sequence=COLOR_STATUS_VALUES)
    fig5.update_layout(template=CHARTS_TEMPLATE)
    html5 = [html.H4('TSTAR ~ RSTAR'),
             dcc.Graph(figure=fig5)]
    fig6 = px.scatter(chart_data, x='MSTAR', y='RSTAR', size='RPLANET',
                       color='status', color_discrete_sequence=COLOR_STATUS_VALUES)
    fig6.update_layout(template=CHARTS_TEMPLATE)
    html6 = [html.H4('MSTAR ~ RSTAR'),
    ...
    html7 = [html.H4('Raw Data'), tbl]

    return html1, html2, html3, html4, html5, html6, html7
